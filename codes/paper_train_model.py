...
# #get all the elements considered in the dataset

# # df['elements'] is a series of list
# l = df['elements'].tolist()
# # flatten the list
...
#         # get the index of df whose 'elements' column contains the element
#         has_ele = df['elements'].apply(lambda x: ele in x)
#         # define the training and test sets
#         id_train = df[~has_ele].index
#         # id_test = df[has_ele ].index
#         id_test = df[has_ele & (df['NIONS']>10)].index
...
#         print('')
#         print('')
#         print(f'{model_name}: leave {ele} out')
#         print(f'train on {X_pool.shape[0]} samples and test on {X_test.shape[0]} samples')
#         print('MAD and STD of the test set')
#         get_mad_std(y_test)
#         print('')
#         model, y_pred, metrics = train_predict(
#             pipe[model_name], X_pool, y_pool, X_test, y_test
#             )
...
# id_test = df[has_ele].sample(frac=0.2, random_state=random_state).index
# # index of training set that does not contain the element
# id_train_not_have_ele = df[~has_ele].index.tolist()
# # index of training set that contains the element and not in the test set
# id_train_have_ele = df[has_ele].index.drop(id_test).tolist()

# # subsample id_train_have_ele 
# id_train_have_ele = random.sample(id_train_have_ele, n_toinclude)

# # combine the two training sets
# id_train = id_train_not_have_ele + id_train_have_ele
...
# # for model_name in pipe.keys():
# #     eval_ood(pipe[model_name],
# #              X_train=X_all.loc[id_bcc], y_train=y_all.loc[id_bcc], 
# #              X_test=X_all.loc[id_fcc], y_test=y_all.loc[id_fcc],
# #              title=f'{model_name}: bcc->fcc')             

# # print('train on fcc and test on bcc')
# # for model_name in pipe.keys():
# #     eval_ood(pipe[model_name],
# #              X_train=X_all.loc[id_fcc], y_train=y_all.loc[id_fcc], 
# #              X_test=X_all.loc[id_bcc], y_test=y_all.loc[id_bcc],
# #              title=f'{model_name}: fcc->bcc')
    
    
# # #%%
# # id_bcc_small = list( set(id_bcc) & set(id_small) )
# # id_fcc_small = list( set(id_fcc) & set(id_small) )

# # print('train on bcc small and test on all fcc')
# # for model_name in pipe.keys():
# #     eval_ood(pipe[model_name],
# #              X_train=X_all.loc[id_bcc_small], y_train=y_all.loc[id_bcc_small], 
# #              X_test=X_all.loc[id_fcc], y_test=y_all.loc[id_fcc],
# #              title=f'{model_name}: bcc small->fcc')
# # print('train on fcc small and test on all bcc')
# # for model_name in pipe.keys():
# #     eval_ood(pipe[model_name],
# #              X_train=X_all.loc[id_fcc_small], y_train=y_all.loc[id_fcc_small], 
# #              X_test=X_all.loc[id_bcc], y_test=y_all.loc[id_bcc],
# #              title=f'{model_name}: fcc small->bcc')

# # #%%
# # id_bcc_large = list( set(id_bcc) & set(id_large) )
# # id_fcc_large = list( set(id_fcc) & set(id_large) )

# # print('train on bcc large and test on all fcc')
# # for model_name in pipe.keys():
# #     eval_ood(pipe[model_name],
# #              X_train=X_all.loc[id_bcc_large], y_train=y_all.loc[id_bcc_large], 
# #              X_test=X_all.loc[id_fcc], y_test=y_all.loc[id_fcc],
# #              title=f'{model_name}: bcc large->fcc')
# # print('train on fcc large and test on all bcc')
# # for model_name in pipe.keys():
# #     eval_ood(pipe[model_name],
# #              X_train=X_all.loc[id_fcc_large], y_train=y_all.loc[id_fcc_large], 
# #              X_test=X_all.loc[id_bcc], y_test=y_all.loc[id_bcc],
# #              title=f'{model_name}: fcc large->bcc')
